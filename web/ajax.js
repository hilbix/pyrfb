...
ajax.send=function(u,f,m,a,h){var x=ajax.x(),t='x-www-form-urlencoded';if(m=='POSTbin'){t='binary';m='POST'};x.open(m,u,true);x.onreadystatechange=function(){if(x.readyState==4)f(x.responseText,x,x.status,x.getResponseHeader("Last-Modified"))};if(m=='POST')x.setRequestHeader('Content-type','application/'+t);if(h)h(x);x.send(a)};
...
ajax.push=function(url,func,args){ajax.send(url,func,'POSTbin',args)};
